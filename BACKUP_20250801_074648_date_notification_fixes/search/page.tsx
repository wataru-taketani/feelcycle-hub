...
export default function SearchPage({ onNavigate }: LessonSearchProps) {
    ...
    try {
      ...
      for (const studioId of selectedStudios) {
        ...
        if (Object.keys(studioGroups).length > 0) {
          ...
          if (!found) {
            console.warn(`Studio not found in API groups, trying fallback: ${studioId}`);
            // API groups でも見つからない場合は静的リストからも検索
            const fallbackStudio = [...eastAreaStudios, ...northAreaStudios, ...westAreaStudios, ...southAreaStudios]
              .find(s => s.id === studioId);
            if (fallbackStudio) {
              studioCode = fallbackStudio.code;
              console.log(`Found studio in fallback list: ${studioId} -> ${studioCode}`);
            } else {
              console.warn(`Studio not found in fallback either: ${studioId}`);
              continue;
            }
          }
        } else {
          ...
          const studio = [...eastAreaStudios, ...northAreaStudios, ...westAreaStudios, ...southAreaStudios]
            .find(s => s.id === studioId);
          if (!studio) {
            console.warn(`Studio not found in static list: ${studioId}`);
            continue;
          }
          studioCode = studio.code;
        }
        
        console.log(`🔍 Searching lessons for studio: ${studioCode} (original ID: ${studioId})`);
        ...
        const url = `${apiBaseUrl}/lessons?studioCode=${studioCode}&range=true&startDate=${startDate}&endDate=${endDate}`;
        console.log(`📡 API Request: ${url}`);
        
        const response = await axios.get(url);
        
        console.log(`📥 API Response for ${studioCode}:`, {
          success: response.data.success,
          hasData: !!response.data.data,
          hasLessonsByDate: !!response.data.data?.lessonsByDate,
          dateCount: response.data.data?.lessonsByDate ? Object.keys(response.data.data.lessonsByDate).length : 0,
          totalLessons: response.data.data?.total || 0
        });
        
        if (response.data.success && response.data.data?.lessonsByDate) {
          const dates = Object.keys(response.data.data.lessonsByDate);
          console.log(`📅 Adding lessons from ${studioCode}: ${dates.length} dates, ${response.data.data.total} total lessons`);
          ...
          Object.keys(response.data.data.lessonsByDate).forEach(date => {
            ...
            const lessonsForDate = response.data.data.lessonsByDate[date];
            allLessonsData[date].push(...lessonsForDate);
            console.log(`  ${date}: ${lessonsForDate.length} lessons`);
          });
        } else {
          console.warn(`❌ No lesson data returned for ${studioCode}:`, response.data);
        }
      }
      
      const totalLessons = Object.values(allLessonsData).flat().length;
      setLessonsByDate(allLessonsData);
      console.log('✅ Search results loaded:', {
        days: Object.keys(allLessonsData).length,
        totalLessons: totalLessons,
        dates: Object.keys(allLessonsData),
        sampleData: Object.keys(allLessonsData).slice(0, 2).map(date => ({
          date,
          lessons: allLessonsData[date].length
        }))
      });
  ...
  const getFilteredLessons = () => {
    ...
    console.log('🔍 getFilteredLessons - 通常の検索モード:', {
      hasSearched,
      lessonsByDateKeys: Object.keys(lessonsByDate),
      lessonsByDateCount: Object.keys(lessonsByDate).length,
      selectedInstructors: selectedInstructors,
      totalLessonsInData: Object.values(lessonsByDate).flat().length
    });
    ...
    console.log('✅ getFilteredLessons result:', {
      allLessonsLength: allLessons.length,
      sampleLessons: allLessons.slice(0, 3).map(lesson => ({
        id: lesson.id,
        date: lesson.date,
        time: lesson.time,
        program: lesson.program,
  ...
  // 動的な日付範囲を生成（今日以降の日付のみ）
  const generateDateRange = () => {
    const todayJST = getTodayJST(); // YYYY-MM-DD形式で今日の日付を取得
    const allDates = Object.keys(lessonsByDate)
      .filter(dateString => dateString >= todayJST) // 今日以降の日付のみフィルタ
      .sort();
    
    console.log('📅 日付フィルタリング:', {
      todayJST,
      allDatesBeforeFilter: Object.keys(lessonsByDate).sort(),
      allDatesAfterFilter: allDates
    });
