...
async function debugFinalScraper() {
    ...
    try {
        ...
        if (lessons.length > 0) {
            console.log('\n📋 取得されたレッスン (最初の20件):');
            lessons.slice(0, 20).forEach((lesson, index) => {
                console.log(`  ${index + 1}. ${lesson.time} | ${lesson.name} (${lesson.instructor}) [${lesson.selector}]`);
            });
            // 07:00のレッスンを探す
            const lesson0700 = lessons.find((lesson) => lesson.time.includes('07:00'));
            if (lesson0700) {
            ...
            const deepDomAnalysis = await page.evaluate(() => {
                ...
                return {
                    ...
                    elementsByTag: Object.entries(elementsByTag).sort(([, a], [, b]) => b - a).slice(0, 10),
            ...
            console.log('主要タグ:');
            deepDomAnalysis.elementsByTag.forEach(([tag, count]) => {
                console.log(`  ${tag}: ${count}個`);
            });
            console.log('\n⏰ 時間らしいテキストを持つ要素:');
            deepDomAnalysis.timeElements.forEach((el, index) => {
