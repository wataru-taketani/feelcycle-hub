...
exports.progressiveDailyRefresh = progressiveDailyRefresh;
...
async function progressiveDailyRefresh() {
    ...
    try {
        ...
        const progress = await studios_service_1.studiosService.getBatchProgress();
        ...
        if (progress.remaining === 0 && progress.processing === 0) {
            ...
            await studios_service_1.studiosService.resetAllBatchStatuses();
            ...
            const studioUpdateResult = await studios_service_1.studiosService.refreshStudiosFromScraping(studios);
        ...
        const studioToProcess = await studios_service_1.studiosService.getNextUnprocessedStudio();
        if (!studioToProcess) {
            console.log('🎉 All studios have been processed today!');
            // Get final progress for reporting
            const finalProgress = await studios_service_1.studiosService.getBatchProgress();
        ...
        // Mark as processing
        await studios_service_1.studiosService.markStudioAsProcessed(studioToProcess.studioCode, 'processing');
        ...
        try {
            ...
            const allLessons = await real_scraper_1.RealFeelcycleScraper.searchAllLessons(studioToProcess.studioCode);
            if (allLessons.length > 0) {
                ...
                // Mark as completed
                await studios_service_1.studiosService.markStudioAsProcessed(studioToProcess.studioCode, 'completed');
            ...
            await studios_service_1.studiosService.markStudioAsProcessed(studioToProcess.studioCode, 'failed', errorMessage);
        ...
        const updatedProgress = await studios_service_1.studiosService.getBatchProgress();
