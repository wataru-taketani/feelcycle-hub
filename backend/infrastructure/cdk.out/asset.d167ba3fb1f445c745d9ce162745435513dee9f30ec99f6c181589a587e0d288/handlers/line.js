 ...
 * LINE関連のリクエストハンドラー
 */
async function lineHandler(event) {
 ...
 * 個別のLINEイベント処理
 */
async function processLineEvent(event, lineService, userService) {
    try {
 ...
 * フォローイベント処理
 */
async function handleFollowEvent(event, lineService, userService, user) {
 ...
 * アンフォローイベント処理
 */
async function handleUnfollowEvent(event, userService, user) {
    if (user) {
 ...
 * メッセージイベント処理
 */
async function handleMessageEvent(event, lineService, userService, user) {
 ...
 * メッセージに対する応答生成
 */
async function generateResponseMessage(messageText, user, userService) {
    try {
        // 残レッスン数問い合わせ
