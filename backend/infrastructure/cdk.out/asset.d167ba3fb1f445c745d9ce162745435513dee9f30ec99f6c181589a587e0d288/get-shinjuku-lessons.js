...
async function getShinjukuLessons() {
    ...
    try {
        ...
        const lessons = await real_scraper_1.RealFeelcycleScraper.searchRealLessons(shinjukuStudio.code, '2025-07-24');
        console.log(`✅ ${lessons.length}件のレッスンを取得しました`);
        if (lessons.length > 0) {
            // DynamoDBに保存
            console.log('\n💾 Step 3: DynamoDBに保存中...');
            for (let i = 0; i < lessons.length; i++) {
                await lessons_service_1.lessonsService.storeLessonData(lessons[i]);
                if ((i + 1) % 20 === 0) {
