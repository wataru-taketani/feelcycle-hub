...
class UpdateProgressMonitor {
  ...
  private maxStableChecks = 5; // 5回連続で同じ件数なら完了と判定
  ...
  async getCurrentStats(): Promise<{
    ...
    try {
      ...
      // スタジオ別統計を取得
      const allItemsResult = await docClient.send(new ScanCommand({
        TableName: LESSONS_TABLE_NAME,
        ProjectionExpression: 'studioCode, lessonDate, startTime, lessonName, instructor'
      }));
      
      const studioStats: { [key: string]: number } = {};
   ...
   * 進捗状況を表示
   */
  async displayProgress(): Promise<boolean> {
   ...
   * 更新完了まで監視
   */
  async monitorUntilComplete(): Promise<void> {
